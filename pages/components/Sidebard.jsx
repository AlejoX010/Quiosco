...
const Sidebard = () => {
    ...
    return (
        <>
            <Image width={300} height={100} src="/assets/img/logo.svg" alt='imagen logo café'/>
            <nav className='mt-10'>
                {categorias.map(categoria => (
                    ...
                    <button key={categoria.id} onClick={()=> handleClickCategoria(categoria.id)} type='button' className={`${categoriaActual?.id === categoria.id ? 'bg-yellow-400':''} w-full flex items-center gap-5 hover:bg-yellow-400 p-5 border`}>
                        <div className='ml-5'>
                        <Image  width={80} height={80} src={`/assets/img/icono_${categoria.icono}.svg`} alt='Imagen icono'/>
                        </div>
